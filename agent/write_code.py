...
def write_code(state: AgentState, files) -> str:
    ...
    for file in files:
        ...
        messages = [
            {"role": "system", "content": prompt.format(TECH_LANGUAGE=state.get('input').get('tech_language'), TECH_FRAMEWORK=state.get('input').get('tech_framework'), REQUIREMENTS=state.get('requirements'), TECH_REQUIREMENTS=state.get('technical_requirements'))},
